#!/usr/bin/env python3
"""
Compare lowess validation results against R's stats::lowess implementation.
...
def compare_implementations():
    # Adjusted paths for lowess repository structure
    base_dir = Path("output")
    r_dir = base_dir / "r"
    lowess_dir = base_dir / "lowess"
    
    if not r_dir.exists():
        print(f"Error: R output directory not found at {r_dir}. Run R/validate.R first.")
        return False
    
    if not lowess_dir.exists():
        print(f"Error: lowess output directory not found at {lowess_dir}. Run lowess/src/validate.rs first.")
        return False
    
    print("=" * 85)
    print("VALIDATION: lowess (Rust) vs R stats::lowess (Reference)")
    print("=" * 85)
    print()
    ...
    for scenario in scenarios:
        ...
        lowess_file = lowess_dir / f"{scenario}.json"
        
        if not lowess_file.exists():
        ...
        with open(r_file) as f:
            r_data = json.load(f)
        with open(lowess_file) as f:
            lowess_data = json.load(f)
        
        # Compare fitted values
        r_fitted = np.array(r_data['result']['fitted'])
        lowess_fitted = np.array(lowess_data['result']['fitted'])
        
        # Ensure lengths match
        if len(r_fitted) != len(lowess_fitted):
             print(f"{scenario:<30} | {'SIZE MISMATCH':<15} | {'-':<15} | {'-':<15}")
             mismatches.append(scenario)
             continue

        diff = np.abs(r_fitted - lowess_fitted)

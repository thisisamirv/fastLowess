#!/usr/bin/env python3
"""
Convert Criterion GPU benchmark results to JSON format.
...
This script extracts timing data and writes it to benchmarks/output/rust_benchmark_gpu.json.

Usage: python3 convert_criterion_gpu.py
"""
...
def find_criterion_results(criterion_dir: Path) -> Dict[str, List[dict]]:
    ...
    for group_dir in criterion_dir.iterdir():
        ...
        # Normalize category name (strip gpu_ prefix)
        raw_category = group_dir.name
        if raw_category.startswith("gpu_"):
            category = raw_category[4:]  # Remove 'gpu_' prefix
        else:
            # Skip non-gpu benchmarks if mixed
            continue
        ...
        for bench_dir in group_dir.iterdir():
            ...
            if new_dir.exists() and (new_dir / "estimates.json").exists():
                # Non-parameterized benchmark (e.g., gpu_pathological/clustered)
                ...
                for param_dir in bench_dir.iterdir():
                    ...
                    if estimates_file.exists():
                        ...
                        if timing:
                            ...
                            # Matches convert_criterion.py convention
...
def main():
    ...
    # Adjust path if needed, usually target/criterion is in the crate root or workspace root
    criterion_dir = script_dir / "fastLowess" / "target" / "criterion"
    ...
    if not results:
        print("No GPU criterion results found (looking for gpu_* folders). Run 'cargo bench --bench benchmark_gpu' first.")
        # Do not allow fail, just print warning
    
    # Write to output file
    output_file = output_dir / "rust_benchmark_gpu.json"
